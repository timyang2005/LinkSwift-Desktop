...
impl QuarkApi {
    ...
    pub fn parse_share_url(url: &str) -> Result<String, AppError> {
        ...
        let after_s = parts[1];
        let id_part = after_s.trim_end_matches('/').split('?').next().unwrap_or("");
    ...
    pub async fn transfer_files(
        ...
        if !response.status().is_success() {
            return Err(AppError::TransferFailed(format!("HTTP {}", response.status())));
        }
    ...
    pub async fn query_transfer_task(&self, task_id: &str) -> Result<TransferTaskStatus, AppError> {
        let url = format!("{}/1/clouddrive/task?pr=1&task_id={}&verify=1", self.base_url, task_id);
    ...
    pub async fn verify_credential(&self) -> Result<bool, AppError> {
        let url = format!("{}/1/clouddrive/file/sort?dir_fid=0&order_by=2&order_sort=1&page=1&size=1", self.base_url);
