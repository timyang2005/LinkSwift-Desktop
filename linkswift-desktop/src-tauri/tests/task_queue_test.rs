...
fn task_queue_list_tasks_returns_all() {
    ...
    queue.add_task(create_test_task("task1", "1", TaskStatus::Pending)).unwrap();
    queue.add_task(create_test_task("task2", "2", TaskStatus::Transferring { progress: 50 })).unwrap();
    queue.add_task(create_test_task("task3", "3", TaskStatus::Completed)).unwrap();
...
fn task_queue_get_pending_tasks() {
    ...
    queue.add_task(create_test_task("task1", "1", TaskStatus::Pending)).unwrap();
    queue.add_task(create_test_task("task2", "2", TaskStatus::Completed)).unwrap();
    queue.add_task(create_test_task("task3", "3", TaskStatus::Pending)).unwrap();
...
fn task_queue_get_tasks_by_status() {
    ...
    queue.add_task(create_test_task("task1", "1", TaskStatus::Pending)).unwrap();
    queue.add_task(create_test_task("task2", "2", TaskStatus::Transferring { progress: 50 })).unwrap();
    queue.add_task(create_test_task("task3", "3", TaskStatus::Transferring { progress: 100 })).unwrap();
    queue.add_task(create_test_task("task4", "4", TaskStatus::Completed)).unwrap();
    
    let transferring = queue.get_tasks_by_status(TaskStatus::Transferring { progress: 50 }).unwrap();
...
fn task_queue_task_count() {
    ...
    queue.add_task(create_test_task("task2", "2", TaskStatus::Pending)).unwrap();
